...
cachedComputerRecord=$(curl -sS -X GET "${url}api/v1/computers-inventory-detail/5" -H "accept: application/json" -H "Authorization: Bearer ${bearerToken}")
getJsonValue "$cachedComputerRecord" 'id'
getJsonValue "$cachedComputerRecord" 'general.name'
getJsonValue "$cachedComputerRecord" 'userAndLocation.position'
